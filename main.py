...
class Note:
    def __init__(self, note_id, title, body, created_at=None, updated_at=None):
        ...
        self.title = title
        self.body = body
        self.created_at = created_at if created_at else datetime.now().strftime("%Y-%m-%d %H:%M:%S")("%d-%m-%Y %S:%M:%H")
        self.updated_at = updated_at if updated_at else datetime.now().strftime("%Y-%m-%d %H:%M:%S")("%d-%m-%Y %S:%M:%H")
